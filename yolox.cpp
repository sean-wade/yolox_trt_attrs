...
static void generate_yolox_proposals(std::vector<GridAndStride> grid_strides, float* feat_blob, float prob_threshold, std::vector<Object>& objects)
{
    ...
    for (int anchor_idx = 0; anchor_idx < num_anchors; anchor_idx++)
    {
        ...
        const int basic_pos = anchor_idx * (NUM_CLASSES + 5 + 50);
...
void save_prob_txt(float* prob, std::string txt_path)
{
...
void save_input_txt(float* input, std::string txt_path)
{
    std::fstream txt_file(txt_path, std::ios::out);
